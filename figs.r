...
get_labels_and_metrics = function(df){
  df$OR = exp(df$Beta)
  df$LO_CI = exp(df$Beta - 1.96*df$SE)
  df$UP_CI = exp(df$Beta + 1.96*df$SE)
...
eczema_exposure_res[eczema_exposure_res$Exposure == "eczema", ]$Exposure = "Atopic dermatitis"
...
eczema_outcome_res[eczema_outcome_res$Outcome == "eczema", ]$Outcome = "Atopic dermatitis"
